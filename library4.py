...
def extractAllWatermark(img, imgMarked, logo, channel=2, xor=True, grayCode=True, perm=True, filename="", typeReg=0, test=False):
    ...
    if test == False:
        fig.tight_layout()
        fig.savefig("assets/test/" + folderOut + "-" + filename + ".png")
        plt.close(fig)
        plt.close()
...
def embeddingWatermark(img, watermark, channel=2, lamb=5, xor=True, grayCode=True, perm=True):
    ...
    fig.tight_layout()
    fig.savefig("assets/test/watermark_steps.png")
    plt.close(fig)
    plt.close()

    cv2.imwrite("assets/test/imageWithWatermark.png", cv2.cvtColor(imgOut, cv2.COLOR_BGR2RGB))
    return imgOut
...
def main_emb(imgPath, watPath, channel=2, lamb=5, xor=True, grayCode=True, perm=True, test=False):
    ...
    cv2.imwrite("assets/test/imageRes.png", cv2.cvtColor(imgRes, cv2.COLOR_BGR2RGB))
    cv2.imwrite("assets/test/watermarkRes.png", cv2.cvtColor(watRes, cv2.COLOR_BGR2RGB))
